...
export async function renderDashboard() {
    ...
    const listHtml = communities.map(c => `
        <a href="#/community/${c.PK.split('#')[1]}" class="block bg-card p-6 hover: transition border border-ink">
            <h3 class="font-bold text-lg">${c.name}</h3>
            <div class="mt-2 text-xs text-ink/40">ID: ${c.PK.split('#')[1]}</div>
            <div class="mt-2">
                ${c.features?.posts ? '<span class="bg-success text-ink border-2 border-ink text-xs px-2 py-1">Posts Enabled</span>' : ''}
            </div>
        </a>
    `).join('');
    ...
    document.getElementById('app').innerHTML = `
        <div class="flex justify-between items-center mb-8">
            ...
            ${isSuperAdmin ? `
                <button onclick="openModal('create-community')" class="bg-slate-800 text-white px-4 py-2 text-sm hover:bg-slate-900">
                    + New Community
                </button>
            ` : ''}
        </div>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ${listHtml || '<p class="text-ink/50">No communities found.</p>'}
        </div>
        ${modalTemplate('create-community', 'Create Community', `
            <form onsubmit="handleCreateCommunity(event)">
                ...
                <input type="text" name="ownerId" placeholder="Owner User ID" class="w-full border p-2 mb-4" required>
